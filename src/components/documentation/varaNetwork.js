  ...
  return (
    <div>
      ...
      <ul>
        ...
        <li>
          <strong>VaraNetwork.Interaction</strong>
        </li>
      ...
      />
      <hr className="my-5" />
      <h3>VaraNetwork.Interaction</h3>
      <p>
        With this element, the user can interact with the VARA Network through
        various functions. Depending on the call made by this element, it can
        read or write information from a contract deployed on VARA, and use this
        information within the component deployed on BOS. This element contains
        several functions that can be called using the trigger property. This
        property allows the user to add HTML code that uses these functions,
        activating their operation. The available functions are as follows:
      </p>
      <ul>
        <li>
      ...
      <h4>readState</h4>
      <p>
        The readState function allows fetching the state of any contract
        deployed on VARA Network. It returns a promise which, if successful,
        provides the requested information. Any errors encountered will be shown
        in the console. readState takes the following parameters:
      </p>
      <ul>
